...
export default function TeamPage() {
  let members: ReturnType<typeof getTeam> = [];
        ...
        {members.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">
              Members will appear here once added via the admin panel.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
            {members.map((m) => (
              <div
                key={m.id}
                className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex flex-col items-center text-center card-hover"
              >
