        ...
        {tab === "team" && (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => setTeam([...team, { id: Date.now().toString(), name: "New", surname: "Member", email: "", image: "", order: team.length + 1 }])}
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-slate-50 cursor-pointer" onClick={() => setOpen(!open)}>
        <span className="font-semibold text-slate-900 text-sm">{member.name} {member.surname}</span>
      ...
      {open && (
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={member.name} onChange={(e) => onChange({ ...member, name: e.target.value })} className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Surname</label>
            <input type="text" value={member.surname} onChange={(e) => onChange({ ...member, surname: e.target.value })} className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Email (optional)</label>
            <input type="email" value={member.email ?? ""} onChange={(e) => onChange({ ...member, email: e.target.value || undefined })} className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Image URL (optional)</label>
            <input type="text" value={member.image ?? ""} onChange={(e) => onChange({ ...member, image: e.target.value || undefined })} className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1 block">Order</label>
            <input type="number" value={member.order} onChange={(e) => onChange({ ...member, order: parseInt(e.target.value) || 0 })} className="w-24 px-3 py-1.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          </div>
        </div>
      )}
    </div>
